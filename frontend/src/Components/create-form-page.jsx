...
export default function NewFormPage() {
  ...
  const submitForm = async () => {
    if (!title) {
      ...
      return alert("Please add some input to the form");
    }
    _setLoading(true);
    const payload = {
      form_title: title,
    ...
    console.log(payload);
    try {
      const newForm = await axios.put(
        `${process.env.REACT_APP_HOST}/create-new-form`,
        payload
      );
      _setLoading(false);

      console.log(newForm.data.code);
      if (newForm.data.code == 201) {
        alert("New Form Added Successfully");
        navigate("/");
      } else {
        alert("SERVER ERROR");
      }
    ...
    return (
      <div className="home-page">
        <div className="jumbotron">
          <h1 class="display-5">Create New Form</h1>
        </div>
